...
Newer ("workbench 2.0") All of Us workspaces don't come with a bucket already set up, so `aou_ls_bucket()` and the other bucket functions below have nothing to point to until you create one. Run this once per workspace:

```{r}
aou_create_bucket()
```

This creates (or resolves, if you've already created one) a bucket named `"workspace-bucket"` and sets `WORKSPACE_BUCKET` for you. You can also create a temporary bucket, whose contents are automatically deleted after a couple of weeks, for intermediate files you don't need to keep:

```{r}
aou_create_bucket(temporary = TRUE)
```

You only need to do this once: the resolved bucket URL is cached to `~/.aou-env`, so future R sessions pick it up automatically without recreating anything. If you ever want to check what's cached (e.g., which buckets you've already created in this workspace), you can read the file directly:

```{r}
cat(readLines("~/.aou-env"))
```
